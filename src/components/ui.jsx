...
export function Icon({ name, size = 20, className = '', ...props }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" className={`icon ${className}`} {...props}>{paths[name] || paths.compass}</svg>;
}

export function Brand() {
  return <span className="brand"><span className="brand-mark"><Icon name="pin" size={23} /></span><span>FIND<span className="brand-amp">&</span>GO</span></span>;
}

export function VenueImage({ src, alt, ...props }) {
  return <img src={src} alt={alt} {...props} onError={(event) => {
    // Tránh lặp vô hạn khi ảnh dự phòng cũng không tải được.
    event.currentTarget.onerror = null;
    if (!event.currentTarget.src.endsWith('/assets/venue-fallback.svg')) event.currentTarget.src = '/assets/venue-fallback.svg';
  }} />;
}

// Hộp thoại gốc giữ focus bên trong và hỗ trợ phím Escape.
export function Modal({ title, children, onClose }) {
  ...
  useLayoutEffect(() => {
    ...
    return () => {
      ...
      // Nút đăng nhập trong menu có thể đã bị gỡ khỏi DOM.
  ...
  return <dialog ref={dialogRef} className="modal" aria-labelledby="dialog-title" onCancel={onClose} onClick={(event) => { if (event.target === event.currentTarget) { const rect = event.currentTarget.getBoundingClientRect(); if (event.clientX < rect.left || event.clientX > rect.right || event.clientY < rect.top || event.clientY > rect.bottom) onClose(); } }}>
    <div className="modal-header"><h2 id="dialog-title">{title}</h2><button className="icon-button" onClick={onClose} aria-label="Đóng hộp thoại" autoFocus><Icon name="x" /></button></div>
    {children}
  </dialog>;
}
